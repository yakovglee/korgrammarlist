...
import { BookOpen, Sparkles, GraduationCap, Search, X, PlayCircle, Eye } from 'lucide-react';
...
const GrammarApp = () => {
  ...
  const [quizCard, setQuizCard] = useState<typeof grammarData[0] | null>(null);
  ...
  const startQuiz = () => {
    const randomCard = grammarData[Math.floor(Math.random() * grammarData.length)];
    ...
    <div className="min-h-screen bg-[#F5F5F7] pb-20 overflow-x-hidden">
          ...
          >
            ...
            >
              <Card className="p-8 shadow-2xl border-none rounded-[40px] bg-white text-center relative overflow-hidden">
                <Button variant="ghost" size="icon" onClick={() => setQuizCard(null)} className="absolute top-4 right-4 rounded-full">
                  <X size={20} />
                </Button>
                ...
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Назови формулу для:</h3>
                <h2 className="text-2xl font-bold text-[#1d1d1f] mb-8">{quizCard.title}</h2>

                <div className="space-y-4">
                  <AnimatePresence mode="wait">
                    {!showAnswer ? (
                      <motion.div key="question" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <p className="text-[#86868b] text-sm italic mb-10">"{quizCard.example.replace(quizCard.title.split(' ')[0], '____')}"</p>
                        <Button onClick={() => setShowAnswer(true)} className="w-full h-14 bg-blue-600 hover:bg-blue-700 rounded-2xl text-lg font-semibold">
                          Показать ответ
                        </Button>
                      </motion.div>
                    ) : (
                      <motion.div key="answer" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        <div className="p-4 bg-green-50 rounded-2xl border border-green-100">
                          <p className="text-green-700 font-mono text-lg font-bold">{quizCard.formula}</p>
                        </div>
                        <Button onClick={startQuiz} className="w-full h-14 bg-[#1d1d1f] rounded-2xl">
                          Следующая
                        </Button>
                      </motion.div>
                    )}
      ...
      <div className="max-w-2xl mx-auto px-6 pt-12">
        <header className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-4xl font-bold text-[#1d1d1f] tracking-tight">Grammar</h1>
            <p className="text-[#86868b]">Master your exam rules</p>
          </div>
          <Button onClick={startQuiz} className="rounded-full bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 gap-2">
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${
                  activeCategory === cat ? "bg-[#1d1d1f] text-white" : "bg-white text-gray-500 border border-gray-100"
                }`}
              >
                {cat}
              </button>
        ...
        <div className="space-y-3">
          {filteredData.map((item) => (
            <Card key={item.id} className="border-none shadow-sm rounded-[24px] overflow-hidden">
              <Accordion type="single" collapsible>
                <AccordionItem value={item.id} className="border-none">
                  ...
                  <AccordionContent className="px-6 pb-6 pt-2">
                    ...
                    <p className="text-sm text-gray-600 italic px-1">"{item.example}"</p>
                  </AccordionContent>
                </AccordionItem>
              </Accordion>
            </Card>
